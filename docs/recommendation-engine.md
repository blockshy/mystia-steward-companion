...
[游戏界面辅助](game-ui-integration.md)；受控稀客如何进入 operational 消费者见
...
参与状态不改变订单捕获、推荐候选、Worker 结果或 primary plan。暂停的受控稀客仍保留这些推荐事实，但不会
投影到“经营中 -> 推荐 -> 稀客”或稀客订单专注模式；重新启用后按 Mod 权威 `queuePosition` 恢复展示。
自动化和游戏 UI 等 operational 消费者在消费 primary plan 前使用同一 participation projection。模块关闭或
有效名单为空时不应用这层展示与执行过滤，全部沿用原有集合和排序。完整集合门禁、队列顺序和非阻塞规则只在
[稀客订单参与队列](rare-order-participation.md)维护。
...
participation 暂停是 Worker 结果之后的展示与执行投影，不是订单或推荐事实的删除信号。单笔订单暂停不能从
Worker 数据集移除该订单、清除已验证结果或另建一套候选管线；稀客队列管理和诊断仍可读取同一份事实。
