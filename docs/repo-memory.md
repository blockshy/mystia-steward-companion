...
- 伴随窗口 `设置` 页负责窗口透明度、焦点切换行为、置顶、主题、手柄导航和稀客专注模式显示数量。透明度通过 Tauri transparent window + CSS 背景 alpha 实现，文字不随背景变淡。
...
- 默认热键 `F8` 和 `RS Click` 的主语义是游戏与伴随窗口焦点切换；伴随窗口聚焦时由 Tauri 前端处理热键并按设置切回游戏。手柄切换需要释放锁存和 1.2 秒后端防抖，防止同一次长按连续 toggle。
