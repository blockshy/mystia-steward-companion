...
- 概览页提供“一键邀请全部可邀请稀客”。实现走游戏日间羁绊邀请链路：只读取当前日间场景 NPC，优先通过 `DayScene.SceneManager.CurrentActiveMapLabel`、`RunTimeDayScene.GetMapNPCs()`、`DaySceneMap.allCharacters` 和场景 `CharacterConditionComponent` 获取候选；如果实时对象为空，则按当前地图反查 `DataBaseDay.GetAllNPCKeys()` / `RefNPC().possibleDestinations`，并用 `RunTimeDayScene.RefTrackedNPCAvailability()` 过滤当前存档和时间下真正可见的 NPC。候选经 `DataBaseCharacter.RefSGuest()` 映射到 `SpecialGuest`；没有当前场景候选时直接失败并返回诊断，不回退到 `GetSpecialGuestsAndMappedGuests()` 全量邀请。后端检查 `StatusTracker.HasNPCInvited()`、`RunTimeAlbum.GetOrGenerateSpecialNPCKizunaLevel()` 和当前等级成功邀请对话包；符合条件后调用 `StatusTracker.RecordInvitedGuest()` 写入今晚邀请名单。该按钮不调用会随机失败并消耗今日尝试次数的 `DaySceneChatSelectionPannel.InviteSpecGuest()`，也不使用 `HasTemptInvited()` 跳过候选，不直接刷客、不推进时间，也不再写 `Story.SpecialGuestControlled`。
