...
- 稀客自动化会在订单状态中锁定料理、加料、酒水、厨具和可接受成品料理 ID 列表；库存变化或推荐排序变化不会让同一订单中途切换目标。完成订单时 C# 会先精确匹配锁定目标料理，若不存在，再允许复用送餐盘中已观察超过 30 秒且属于该订单可接受成品列表的堆积料理，并在步骤中输出“复用堆积料理”诊断。
