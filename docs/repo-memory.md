...
- 实验性自动化由设置页总开关启用，经营中页提供子选项：自动完成订单、自动取酒、自动开始料理、自动收取料理、只处理收藏配方、出错暂停。开启自动开始料理后，可选择跳过 QTE、打开原生料理 QTE，且只有在原生 QTE 模式下显示自动模拟成功（实验性）。自动化只处理当前排序第一笔稀客订单；临时失败应继续等待并重试，非临时失败才按配置暂停。
