...
- 概览页提供“一键邀请全部可邀请稀客”。实现使用游戏原生 `Story.SpecialGuestControlled` 单例、`CheckSpecialGuestUsability` 和 `RunTimePlayerData.SetControlledSpecialGuests`，只向现有受控稀客列表追加新的 `PreControl` 条目，不清空原列表，也不调用夜间直接刷客入口。若当前页面时机导致具体可用性全部不可判定，后端会调用原生 `ControlScheduled()` 追加空邀请名额，让游戏在经营准备回调中自行选择。该按钮需要存档加载且游戏原生邀请系统已初始化；它写入邀请队列或名额，具体出现仍由游戏经营准备流程处理。
