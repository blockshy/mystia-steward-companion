...
- 概览页提供“一键邀请全部可邀请稀客”。实现走游戏日间羁绊邀请链路：优先读取当前日间场景 NPC，经 `DataBaseCharacter.RefSGuest()` 映射到 `SpecialGuest`；没有场景候选时回退到 `GetSpecialGuestsAndMappedGuests()`。后端检查 `StatusTracker.HasNPCInvited()` / `HasTemptInvited()`、`RunTimeAlbum.GetOrGenerateSpecialNPCKizunaLevel()` 和当前等级邀请对话包，再调用 `DaySceneChatSelectionPannel.InviteSpecGuest()` 复用游戏成功率判定；成功后调用 `StatusTracker.RecordInvitedGuest()`。该按钮不直接刷客、不推进时间，也不再写 `Story.SpecialGuestControlled`。
