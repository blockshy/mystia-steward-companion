...
- 概览页提供“一键邀请全部可邀请稀客”。实现使用游戏原生 `Story.SpecialGuestControlled` 单例、`CheckSpecialGuestUsability` 和 `RunTimePlayerData.SetControlledSpecialGuests`，只向现有受控稀客列表追加新的 `PreControl` 条目，不清空原列表，也不调用夜间直接刷客入口。该按钮需要存档加载且游戏原生邀请系统已初始化；它写入邀请队列，具体出现仍由游戏经营准备流程处理。
