...
- 伴随窗口 `设置` 页负责窗口透明度、焦点切换行为、切换冷却时间、置顶、主题、手柄导航、缺失厨具过滤、实验性游戏界面置顶、目标厨具高亮和实验性自动化总开关。透明度通过 Tauri transparent window + CSS 背景 alpha 实现，文字不随背景变淡；稀客专注模式的料理/酒水显示数量在专注模式浮层内调整。
...
- 实验性自动化由设置页总开关启用，经营中页提供子选项：自动完成订单、自动取酒、自动开始料理、自动收取料理、只处理收藏配方、出错暂停。开启自动开始料理后，可选择跳过 QTE、打开原生料理 QTE，且只有在原生 QTE 模式下显示自动模拟成功（实验性）。原生 QTE 打开成功后由游戏面板负责料理倒计时；自动模拟成功会直接尝试结算成功奖励并跳过面板，避免重复结算。自动化只处理当前排序第一笔稀客订单；临时失败应继续等待并重试，非临时失败才按配置暂停。
