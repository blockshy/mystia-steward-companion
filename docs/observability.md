...
本文说明聚合日志、游戏状态诊断、BepInEx 控制台和诊断包的职责。游戏数据本身如何采集见
[游戏数据提供器](runtime-provider.md)，本地 API 的认证和暴露范围见[本地 API](local-api.md)。
...
- 限量：限制缓存项、文本长度、文件大小、文件数量和导出末尾行数。
- 可关联：保留时间、通道、来源、线程、经营轮次、订单跟踪标识/实例等现有标识。
- 可去重：对高频重复状态使用定长签名或周期摘要，不无限刷屏。
- 失败隔离：写日志失败不能把异常抛回游戏主循环。
- 不猜测：缺少游戏字段时记录明确失败原因，不用显示文本或对象名拼出业务标识。

## 聚合 Mod 日志

`mods/bepinex/src/Save/AggregateModLogService.cs` 提供可选的聚合日志。它注册唯一 `ILogListener`，收集
BepInEx 日志源中的 Mod、自动化和游戏状态消息，并写为带时间、通道、来源、级别和线程信息的结构化
文本。
...
新的有效段，并重置与旧文件相关的去重签名。实现不依赖文件监视器或高频轮询，也不为“等待日志”预建空
文件。
...
- 面向业务的规范快照，只包含通过全部完整性检查的数据。
- 面向定位的限量诊断，记录 Hook 就绪、形态失败、经营轮次、对象标识、状态机与拒绝原因。

第二类不得被页面或自动化反向用作备用数据源。诊断开关只改变采样和日志详细度，不得开启另一套更宽松的
游戏数据读取。任务专题的业务/诊断边界见[任务系统](missions.md)，自动化状态机见
[自动化运行时](automation-runtime.md)。

订单使用进程内单调跟踪标识（稀客 `R-*`、普客 `N-*`）以及精确订单实例与原生键关联跨组件记录。新增
日志时优先携带这些既有标识，不要引入仅凭客人名、桌号或显示文本关联订单的额外路径。

前端可通过受限诊断端点追加“推荐为何无法执行”等决策摘要。服务会校验客户端、签名、行数和文本长度；
前端诊断异常仍不得影响原 API 响应。
...
只为本次由 Mod 新建的窗口禁用系统菜单关闭命令，避免关闭窗口导致日志后端状态损坏。服务分别维护驱动、Win32
窗口存在性和可见性三种状态分离，并确保唯一 `ConsoleLogListener`。

隐藏操作只调用 `ShowWindow(SW_HIDE)`，不分离控制台、不销毁驱动。显隐、启动偏好和状态读取在同一锁
下提交，失败时尽量恢复调用前状态。UTF-8 设置只作用于当前已确认的控制台。
...
- 清单和当前规范快照。
- 游戏静态目录快照。
- 任务、调度器、可接取任务来源和 ServeInWork 等诊断 JSON。
- 聚合日志各段按数量限制的末尾内容（存在时）。

导出只复制限定数量的内容，不暂停业务服务，也不修改源日志。单个源缺失时应在其边界内处理，不能因此生成一个
...
`/logs/export-diagnostics?open=true` 只校验 Token，尚未限制为回环地址；这是现有安全限制，不能把 LAN 暴露给
