...
- 手动检测、下载、打开安装程序和打开项目 Release 页动作。
...
窗口使用标准 Win32 控件键盘导航，支持 Tab、Shift+Tab、Enter、Space 和 Escape。状态摘要与可选择、
滚动、复制的完整详情分开显示，错误不会被截在单行状态栏中。
目标版本取自 updater 的编译版本，显示前逐字节确认当前 runner 与暂存包中的 updater 一致。旧 Mod 会从新
暂存包启动新 updater，因此版本展示不增加启动参数，也不从目录名推断版本；发布时仍必须统一各产品版本。

安装流程为：

1. 展示目标版本并等待用户开始。
2. 请求伴随窗口退出以释放程序文件，提示用户保存进度并手动退出游戏；更新程序只观察退出状态，不主动关闭或强制结束游戏。
3. 将当前插件目录移动到备份目录。
4. 把已验证暂存内容移动到正式目录并复核结构。
5. 以原子方式持续写入状态、消息和进度，供下一次 Mod 启动读取。
6. 失败时尽力恢复备份，并写入可展示的失败状态。

准备和等待退出阶段允许取消后续安装。取消请求与进入文件替换共用一次互斥决议；进入替换后不接受取消。
取消不会自动恢复已关闭的伴随窗口。窗口只有在安装线程发布终态并结束后才能关闭，不能直接丢弃安装线程。
Windows 仅申请 `SYNCHRONIZE` 权限并保留启动时打开的进程句柄，使用原生等待结果观察退出；无法读取状态时
停止安装，不以进程列表文本或固定延时推断退出。启动协议只提供 PID，首次打开前的 PID 重用无法证明原始
进程身份，因此更新程序没有对该 PID 发出窗口关闭或终止进程的能力；PID 重用可能延长等待，用户仍可取消。

安装状态通过本次创建的临时文件、刷盘和原子重命名发布。写入失败会显示错误；若旧目录已备份而新目录尚未
移入，失败路径恢复旧目录。已存在的备份目录会阻止安装，更新程序不会改名覆盖它。

更新程序不从网络下载文件。它接收 `UpdateService` 已校验的暂存路径，在替换前检查目标目录名，并在替换
前后检查最小文件集合；包的长度、SHA-256 和 ZIP 安全性仍由启动它的 `UpdateService` 负责。
