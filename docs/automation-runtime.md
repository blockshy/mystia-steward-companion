...
`ModWorkbench` 只组合依赖和页面入口。自动化控制器随工作台常驻，切换页面不会卸载调度、续租或运行事件确认：

- `useAutomationState` 保存订单执行状态、请求轮次和有界诊断状态；页面可见性只控制诊断的 React 发布。
- `useAutomationControl` 管理控制权获取、续租、释放、连接绑定和配置变更时的请求失效。
- `useOrderAutomation` 对账快照与运行事件，处理人工确认、重试和本场经营切换。
- `useAutomationSchedulers` 负责稀客、普客的独立调度、厨具预约和异步命令；每次响应仍复核请求轮次与事件序号。
- `domain/automation-target.ts` 决定执行目标资格，`automation-request-plan.ts` 投影阶段请求，`automation-lifecycle.ts` 维护精确订单与料理任务的状态归并规则。领域函数不持有页面或网络状态。

控制权申请在等待前一笔传输结束后，必须重新核对连接地址、Token、连接修订、游戏会话、设备权限与控制配置，
过期的排队申请不能发送。人工确认也绑定该连接范围；跨连接或暂停恢复不会提前释放在途确认的传输占用，
旧成功与旧失败都不得修改新范围的错误、订单状态或安全屏障。

新普客特殊经营动作只接受当前输入和上下文的推荐结果；计算中、过期或计算失败时不把保留结果用于新命令。目标本身还必须匹配当前挑战、归属、经营轮次、目标修订号、签名、匹配模式和标签集合，不能给旧配方补上新策略标识。

已经准入的料理任务通过订单状态保留原执行目标。只要经营轮次与目标策略仍匹配，推荐重新计算或失败不会丢弃其锁定目标；目标轮换仍由现有料理任务、回退和人工交接规则决定后续动作，不能另开一锅代替未处理的旧任务。
