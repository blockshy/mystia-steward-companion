...
`RuntimeSpecialBusinessContextService` 读取 `NightSceneDirector.ChallengeMode`，挑战显示名来自 `NightSceneDirector.ChallengeType` 枚举成员的 `InspectorNameAttribute`。它是固定中文元数据，不随游戏语言切换。元数据不可读时只发布原始挑战类型诊断，不使用自建名称表或语言 API 合成标题。
...
HUD Hook 只被动记录挑战已显示的目标：营业额、符卡数、阶段、料理标签、进度、怒气和剩余时间。捕获值只在当前挑战类型与来源匹配时有效；切换场景必须清除旧值。

HUD 目标不是订单标识，也不直接写入普通 `RecommendationState`。营业额、符卡数和未确认结算条件只展示；只有已确认影响评价或挑战进度的料理条件才能进入特殊经营策略。

## 怪诞料理大赛

相关数据类型为 `GameData.Profile.DLC2_KoishiBossData`。

- 第一阶段按普客料理和酒水的喜好标签总命中数改判：`0 -> Exbad`、`1 -> Bad`、`2 -> Normal`、`>= 3 -> Good`；达到 `Good` 增加挑战分。
- 第二阶段只有原始评价为 `ExGood` 时继续检查当前轮换料理标签，再按游戏数据计分。
- 第三阶段分身接受传入的 `Normal`、`Good` 或 `ExGood`。命中当前目标标签时改为 `ExGood`，未命中时改为 `Normal`；“游戏要求必须达到 ExGood”是不正确的结论。
- 古明地恋本体的数据包含护盾、揭示的正面/厌恶料理标签、酒水标签、投食目标分和等级效果数组。数值来自当前游戏资产，不在代码或文档中写死。
- `guestId=2006` 不能单独区分本体与分身；分身具有 `GhostInChallenge` 生成类型，本体还需要阶段、订单形态和控制器证据。
...
- `SpecialOrder` 使用标准标签点单评价：料理/酒水点单建立基础 `Normal=2`，再按最终完整标签中的额外喜好/厌恶调整。
- 精确料理/酒水 `NormalOrder` 先由原料理和原酒水匹配建立 `Normal=2`，后续只统计实际生效的厨具、价格、大份，以及相对原配方新增加料所产生的标签；原料理基础标签不能重复计分。

结果路径保留传入评价：`Good` / `ExGood` 按运行时配置推进，`Normal` 清理订单但不推进，负面评价可能触发厨具锁定等处罚。具体数值不写死。

## 寻找瑞灵踪迹与月都试炼

四场共用 `GameData.Profile.DLC5_MizuchiChallengeBossData` 的订单和评价回调捕获对象。

- `Story_Mizuchi` 使用目标材料噗噗呦果 `5002`，异常控制类型从酒水、料理、对话三类中选择。
- 月都试炼 1/2/3 使用目标材料辣椒水 `5005`，控制类型分别固定为错误料理、错误酒水和错误对话。
- 被控制稀客的原始料理/酒水标签不会被异常表现替换，仍是订单成立的第一层条件。
- 捕获只检查最终 `Food.Modifier` 是否包含目标材料；基础配方自带同材料不算，必须作为额外材料加入。
- 当前被控制对象以 `currentGuestWhoIsControlledByMizuchi` 为准。合法稀客 ID 包含 `0`；只有 `-1` 表示无当前目标。
- `controlled=-1 && control=None(3)` 是开场及每次捕获后的正式无目标保护期。活动期要求被控制稀客的 ID 非负，且控制类型与当前挑战契约一致。
- 唯一评价委托及其捕获对象必须把所选稀客、分组控制器、订单中的稀客与控制器中的稀客绑定到同一原生对象。
- Mod 不调用评价委托，不修改捕获数、Moon Eye、QTE 或阶段推进。

## 血池地狱

相关数据类型为 `GameData.Profile.DLC1_YuumaBossData`，挑战类型为 `Story_BloodPondHell`，BOSS 规范 ID 为 `1003`。

- 第二阶段同时可能生成 `NormalOrder` 和 `SpecialOrder`，第三阶段生成 `NormalOrder`。回调声明为 `OrderBase` 不代表具体形态未知。
- 原订单始终是第一项必要条件：`SpecialOrder` 要求实际送达的料理和酒水满足两项原始标签；`NormalOrder` 要求原料理和原酒水匹配。
- HUD 每轮发布两个料理目标标签，语义是同时满足；只命中一个不算完整命中。
- 原订单不满足时进入最低评价并触发对应怒气和游戏效果；原订单满足而双标签未全中时保留普通评价和较低伤害；全部满足时进入最高评价和较高伤害。
- 伤害、怒气、阶段阈值、时间和目标刷新来自运行时资产。Mod 不主动调用这些推进或伤害入口。
- 事件可锁定或移除厨具；锁定实体不是可用厨具，也不能读取旧控制器能力推断容量。
