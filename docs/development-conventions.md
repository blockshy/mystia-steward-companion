...
- 自动化状态机只把实际动作视为进展，例如取酒、开锅、收取、写入订单或触发评价；不要把“选择订单”“匹配订单”这类前置成功当作进展，否则会掩盖真实卡住。稀客目标料理/酒水长时间未进入送餐盘时允许按订单状态回退到上一实际步骤重新执行，超过回退上限再暂停。普客目标料理长时间未进入暂存容器时，应先重新询问后端 pending 待收取任务；后端仍报告已在制作或等待收取时，前端刷新等待时间并清理旧回退计数，不得直接暂停。只有 pending 消失且需要重新开锅时，才重新发起普客制作。C# pending 收取、开锅成功/失败和 pending 移除需要写入 `BepInEx/config/MystiaStewardCompanion/automation-jobs.log`，诊断日志必须有大小上限并且不能影响游戏流程。
