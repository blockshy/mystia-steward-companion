...
- `概览` 页的一键邀请稀客只能写入游戏原生 `Story.SpecialGuestControlled` / `RunTimePlayerData.SetControlledSpecialGuests` 受控稀客队列，并使用 `CheckSpecialGuestUsability` 判断当前可邀请性；不得直接调用夜间刷客入口或绕过游戏经营准备流程。该功能应保留现有受控稀客状态，只追加新的 `PreControl` 条目，失败时不得清空游戏原列表；如果当前时机无法判出具体可用稀客，可以调用原生 `ControlScheduled()` 新增空邀请名额，让游戏在经营准备回调中自行选择。
