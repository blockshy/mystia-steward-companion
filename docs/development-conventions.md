...
- `概览` 页的一键邀请稀客必须走日间羁绊邀请链路，不再写 `Story.SpecialGuestControlled`。候选优先来自当前日间场景 NPC，使用 `DataBaseCharacter.RefSGuest()` 映射到 `SpecialGuest`；没有场景候选时才回退到 `DataBaseCharacter.GetSpecialGuestsAndMappedGuests()`。每个候选必须检查 `StatusTracker.HasNPCInvited()`、`StatusTracker.HasTemptInvited()`、`RunTimeAlbum.GetOrGenerateSpecialNPCKizunaLevel()` 和当前等级邀请对话包，再调用 `DaySceneChatSelectionPannel.InviteSpecGuest()` 复用游戏原生成功率判定。只有判定成功后才能调用 `StatusTracker.RecordInvitedGuest()`。该功能不得直接刷客、不得推进日间时间、不得修改受控稀客队列。
