...
- 夜间经营订单只允许在 Work 场景且精确经营生命周期处于 Active generation 时读取。Active 期间优先使用 `SpecialOrderRuntimeCapture` 运行时捕获缓存；捕获缓存为空、需要初始化/回退校验，或捕获缓存有订单但本轮可接受订单少于缓存数量时，才把 OrderController、HUD、服务面板和桌位控制器反射结果用于业务缺失项补充。诊断开启时可以额外采样这些来源，但诊断样本只能进入日志快照，不能改变正式订单集合。控制器扫描仍要读取活动稀客和预算资金信息。
...
- 经营中订单排序支持 `点单顺序` 和 `稀客分组`。默认必须保持点单顺序；稀客分组模式下，同一稀客订单放在一起，稀客组之间按该稀客最早订单出现时间排序，组内仍按点单先后排序。经营中列表、当前点单推荐、专注模式、游戏界面置顶目标和自动化第一单选择必须复用同一排序函数。特殊经营可以在订单层跳过没有主执行计划的订单，但选中订单后页面、自动化和游戏界面辅助只能消费该订单的 `executionPlans[0]`。普客特殊经营自动化目标选择是候选搜索级工作，必须通过经营中订单推荐 Worker 预计算并按订单 key 复用，自动化 tick 只能消费结果，不能在 UI 主线程逐单同步调用完整目标搜索。
...
- 经营中读取到已摆放厨具快照时，`排除缺失厨具` 可过滤当前场景没有对应厨具的料理；读不到快照或无法映射厨具名时不得误删推荐。料理厨具类型以游戏 `CookSystemManager` 的 `AllAvailableCookerType` 为准，前端只消费本地 API 给出的中文厨具名。设置页的 `同基础料理显示` 控制同一基础料理在稀客页和经营中推荐中最多展示多少个加料变体；该限制只裁剪主计划之外的 UI 展示行。经营中必须先从完整料理/酒水候选构造并排序执行计划，以 `executionPlans[0]` 作为唯一主执行计划，再把主计划料理和酒水投影为页面首项。自动化开始时、游戏界面置顶、列表高亮和厨具高亮只消费这一个主计划，不得扫描隐藏的后续计划。自动化收藏限定只有在总开关、对应料理/酒水阶段和当前订单自动化权限都开启时才参与主计划归一化，并且必须在计划数量截断前处理；没有匹配计划时不执行对应自动化动作，不得建立兼容性旁路。非可见经营中页的后台推荐只能返回业务需要的紧凑结果，并限制 worker 候选缓存和回包重复更新。
- `游戏界面置顶推荐` 和 `目标厨具高亮` 是两个独立开关。料理、材料和酒水列表项的黄色脉冲属于 `enabled` 对应的置顶功能，不是第三个开关，也不得受 `highlightEnabled` 厨具开关控制。两者共享目标订单的 `executionPlans[0]`，本地 API 必须分别传递 `enabled` 与 `highlightEnabled`，同时携带当前正数 `businessGeneration`；`recipeId` 必须是游戏 `Recipe.Id`，不能用成品 `foodID`。Mod 只接受与当前 Active generation 完全相等的目标，Closing、Destroyed 和上一场请求必须失败。伴随窗口只允许把与当前推荐输入签名一致的 Worker 结果更新为新目标：pending 期间保留上一有效目标，error 时发布空目标并锁存失败 revision，只有后续新的 current 成功 revision 才能解除，任一开关关闭都要立即下发而不能被 freshness 门禁阻止。`ok:false` 或网络失败不能记为成功，必须有上限退避重试；Tauri 原生请求不可取消，发布器必须保持单写者并合并为最新 desired 状态。发布去重边界必须包含 endpoint、token、连接代际和经营 generation。
...
- 稀客料理/酒水展示排序由伴随窗口设置页的置顶开关、自定义推荐料理置顶和推荐权重共同控制。硬过滤后先应用任务料理、自定义推荐料理、收藏料理、收藏酒水置顶，再由 `RecommendationSortProfile` 的启用项、权重、方向和预设共同决定顺序；默认均衡预设综合稀客偏好、厌恶风险、加料数量、资源压力、成本、利润、酒水库存和当前厨具可做。新增排序或置顶规则时必须同时接入稀客页、经营中页、专注模式、缓存签名和唯一主执行计划，不要让页面首项、自动化初始锁和游戏界面辅助出现不同目标。
...
- 已捕获且仍能匹配当前稀客，或仍能确认原运行时订单对象由原 controller 持有的订单，不得使用短时间缓存过期清理；`IsFullfilled` 只表示料理与酒水已送齐，不能作为删除条件。订单只应在明确移除、评价或手动订单结束、稀客离场或长时间硬上限后消失。
...
- 幽幽子第二阶段只在稀客标准评价达到 `ExGood` 并触发正面符卡后推进；候选必须满足当前稀客料理/酒水点单、避开该稀客自身的厌恶，并在排除点单 Tag 后命中至少 `2` 个额外喜好 Tag。不得把幽幽子的固定厌恶 Tag 或等级合计门槛用于第二阶段。第三阶段 `progress` 目标必须保留原订单、避开幽幽子的挑战厌恶 Tag，并要求料理与酒水等级合计至少为 `5`；`refresh` 只清理精确原订单且不承诺推进。进入原生评价查找前先用 Completion 精确对象检查 `IsFullfilled`，未送齐只返回正常等待。NativeEvaluation 优先复核 capture 的强身份、controller 所有权、fulfilled、已送达目标和回调，失败后才让 manager 扫描候选通过同一验证器；不得把 manager 可枚举性当作 capture 存活的必要条件。剧情版只复用最终选中的同一原生 order/controller 捕获记录中的 `onEvaluate` 调用 `EvaulateManualOrder`，重修版确认当前 controller 的 `_50` / `_70` 进度回调后调用 `EvaluateOrder()`；对应回调、送达目标或阈值不可确认时必须暂停。
...
- 运行时稀客订单捕获只能长期显示仍匹配当前活跃稀客，或仍能确认原运行时订单对象由原 controller 持有的订单；无法确认仍在场的捕获项只允许短暂宽限，避免跨伴随窗口重启或跨天残留。最早进入 Closing 的精确生命周期边界时必须同步清空捕获缓存；场景名变化只可作为读取门禁，不能再拥有第二套延迟清理逻辑。经营中稀客订单行的删除按钮调用 `/orders/rare/dismiss` 清理插件端缓存，不应只做前端隐藏。自动送达前若捕获对象仍由同一 controller 持有、尚未满足且身份一致，则 `GuestsManager` 末尾暂时不可枚举不能单独否定该捕获对象；完成定位允许同一对象 `IsFullfilled=true`，直到明确移除生命周期清理。特殊经营原生评价仍需独立复核回调和已送达目标，但复核不得无条件丢弃已证明所有权的 capture；古明地恋本体要求 manager live controller 的现有专用路径不因此改变。
- 稀客订单必须把身份、目录身份与展示分离。`SpecialOrder.RequestFoodTag` / `RequestBeverageTag` 原始数值连同桌位和独立 `runtimeGuestId` 构成捕获、实时扫描和跨帧 target 共用的强身份；归一化 `guestId` 只供推荐与特殊经营规则使用。`AllOrders` / `AllOrdersData` / `PeekOrders()` 暴露的 `OrderBase` 必须先通过共享 IL2CPP cast 入口规范为 `SpecialOrder` 再读取原始 Tag，捕获、Provider 和自动化匹配不得复制不同的 cast 实现。合法负数必须原样保留，读取失败使用 nullable / `Has*TagId=false` 显式表达，任一必要身份缺失或冲突时 fail-closed。`SpecialGuestsController.GetOrderFoodText/GetOrderBevText` 是会应用订单 override 的最终展示文本，其原文保留在捕获诊断；页面优先按原始 ID 显示规范 Tag，映射缺失时才把捕获文本规范化后用于显示，但任一文本都绝不参与相等、包含、宽松订单匹配或料理 job 去重。Delivery 定位必须拒绝 `IsFullfilled=true` 以避免重复送达，Completion/NativeEvaluation 定位必须允许该状态。不得用 `foodRequest` / `beverageRequest` 字段猜测身份，也不得因为静态目录禁止未消费语言探针而删除订单 UI 正式使用的 getter。
