...
- 普客订单自动化必须建立在只读诊断可识别订单的前提下。常规普客快照先用 `OrderController.GetShowInUIOrders()` 和 HUD `OrderingElement.ActiveOrder` 判断 live 可见订单，再把仍匹配 live 订单 key 或桌位/料理/酒水槽位的 `NormalOrderRuntimeCapture` 记录合并回来补充 `GuestGroupController` 绑定；不再存在于 live 订单源的捕获缓存必须剔除，避免经营中页残留旧订单。捕获为空、捕获 Hook 尚未可用或需要诊断/启动校验时，才复用健壮的 IL2CPP 枚举与单例查找逻辑读取 `GuestsManager` 的 Presented/Desk/DeskMap/Repellable/ManualDesk 控制器、Queue 控制器以及 `NightSceneDirector.controlledGuest` 手动控制器中的 `AllOrders`/`AllOrdersData`/`PeekOrders()`。当前普客执行入口按首次出现顺序并发处理未满足订单，并发上限来自 `CompanionPreferences.autoNormalConcurrency`；已经开始制作、等待 pending 直送或正在送达/评价的订单不得继续占用新的开锅调度名额，但“已开始制作”不能作为永久终态，长时间未直接送达时必须允许恢复调度。普客自动化需要独立于稀客节拍快速轮询，并在普客订单 key、料理、酒水或送达状态变化时用短防抖合并触发复查，避免快照连续刷新造成多轮同步重跑。普客阶段包括送达酒水、自动开始料理、自动送达料理和自动完成订单；可见阶段必须由 `autoNormal*` 子开关独立控制，默认关闭。目标料理完成后必须直接送达目标订单，不再写入 `IzakayaConfigure.StoreFood()` 或扫描 `StoredFoods` 作为自动化中转；若最终出锅成品不是目标料理，必须作为恢复动作通过 `IzakayaConfigure.StoreFood()` 放入保温箱，并发布 `cooking-mismatch-stored` 自动化运行时事件驱动前端清理对应订单状态后重新调度，不能只依赖日志或普通步骤码，也不能用反射虚空清理。送达提交必须通过统一运行时送达 helper 完成 `Served*InAir`、顾客桌面显示和最终 `Serv*` 同步；`get_IsFullfilled()` 只表示料理和酒水都已送达、订单可以评价，不代表顾客流程已结束，实际完成状态必须以 `HasEvaluated` 或订单从运行时移除为准。只有 `get_IsFullfilled()` 为真时才能调用 `EvaluateOrder()`。同一订单已有 pending 料理时只能等待，不能因场景里有多个同类厨具而重复制作；pending 直送应优先用 `orderKey` 匹配，但普客运行时对象可能被游戏重建，严格 key 失效时必须要求桌号、料理和酒水仍一致后再重匹配当前订单。稀客和普客开锅前必须共用前端同一轮厨具预约表，按已摆放厨具快照计算同类厨具容量；容量不足时优先保留普客待处理订单，稀客只能等待料理开锅但仍可继续处理送酒或完成订单。
- 出锅直送 pending 的正常路径仍必须直接送达目标订单；只有最终成品不匹配，或已出锅成品的目标订单/可执行控制器连续不可达并达到自动释放阈值时，才允许通过 `IzakayaConfigure.StoreFood()` 放入保温箱并释放 pending。该恢复动作必须发布 `cooking-mismatch-stored` 运行时事件，让前端按同一套清理逻辑回到确认料理阶段；保温箱写入失败时不得移除 pending。
...
- 厨具出锅结果必须读取 `CookController.Result` 或其精确 backing field，并确认对象是料理 `Sellable` 后才能送达或传给 `IzakayaConfigure.StoreFood()`。不得通过大小写退化读取 `CookController.result` / `resultVisual`，这些字段是视觉 `SpriteRenderer`；非 `Sellable` 结果属于结构性错误，应记录来源类型并释放对应 pending，而不是无限重试或调用 `StoreFood`。
- 自动化状态机只把实际动作视为进展，例如送酒、开锅、单项送达提交或触发评价；不要把“选择订单”“匹配订单”这类前置成功当作进展，否则会掩盖真实卡住。稀客和普客完成订单都必须允许料理和酒水按阶段分别送达，只要目标项已进入订单就应通过 `hasServedFood/hasServedBeverage` 校准前端状态；只有订单 `get_IsFullfilled()` 为真时才能触发 `EvaluateOrder()`，但本地状态不能把该字段当成已完成，应继续等待 `HasEvaluated` 或订单消失。C# pending 直送、开锅成功/失败、pending 移除、经营诊断和运行时固定数据诊断都必须接入总日志 `aggregate-mod.log`；连续相同自动化日志必须合并为 `repeat` 摘要。订单相关自动化日志必须带 Mod 运行期订单追踪标识 `trace=R-0001` / `trace=N-0001`，且经营中订单行和自动化诊断必须显示同一标识，便于按单笔订单检索总日志。伴随窗口日志页只保留总日志开关、总日志目录、文件上限和诊断包导出，`/logs/export-diagnostics` 导出的诊断 zip 包含 snapshot 和总日志尾部；诊断包必须有大小上限并且不能影响游戏流程。总日志默认关闭，启用后捕获 BepInEx 全局日志源并每 10 MB 分片，默认保留 30 个文件且允许用户调整；总日志写入、分片和裁剪失败同样不能影响游戏流程。
