...
- 自动化能力是实验性功能，必须由设置页总开关控制；总开关关闭时经营中页不显示自动化配置，也不执行任何自动化动作。稀客自动化只处理当前排序第一笔稀客订单；普客订单处理必须额外由经营中自动化面板的“启用普客处理”子开关启用，开启后不保留手动处理按钮，由伴随窗口轮询自动执行。稀客阶段配置使用 `autoPrep*`，普客阶段配置使用 `autoNormal*`。普客只保留开始料理、收至保温箱、QTE 和出错暂停，不要恢复取酒、直送订单或完成订单开关。子选项默认关闭但记忆用户上次配置。
