...
- 稀客/经营中主推荐必须先满足点单料理 Tag 和酒水 Tag；不满足点单的 fallback 只能作为明确标注的“喜好备选”，不得进入收藏置顶、自动化或一键订单使用的正式推荐数组。料理推荐优先 `foodScore >= 3`，但必须保留“满足点单且低于 3 分”的候选作为正式兜底。
...
- 普客订单自动化必须建立在只读诊断可识别订单的前提下。经营中页诊断和执行路径都需要复用健壮的 IL2CPP 枚举与单例查找逻辑，至少读取 `OrderController.GetShowInUIOrders()`、HUD `OrderingElement.ActiveOrder`、`GuestsManager` 的 Presented/Desk/DeskMap/Repellable/ManualDesk 控制器以及 Queue 控制器中的 `AllOrders`/`AllOrdersData`/`PeekOrders()`。当前普客执行入口按首次出现顺序并发处理最多 3 笔仍需启动料理的未满足订单；已经开始制作、等待 pending 自动收取或已收至保温箱的订单不得继续占用新的调度名额，但“已开始制作”不能作为永久终态，长时间未收进暂存容器时必须重新询问后端并恢复调度。普客自动化只保留自动开始料理和料理完成后写入游戏料理暂存容器 `IzakayaConfigure.StoreFood()` 两步；保温箱不是 `CookController` 空位，不要再扫描厨具控制器或调用 `CookController.Store()`。不得直送酒水，不得写入 `ServFood/ServBeverage/ServedFoodInAir`，不得调用 `EvaluateOrder`、`CookController.AfterPlayerExtract` 或补写送达副作用。每笔普客订单需要独立记录已开始料理、已收至保温箱和暂停状态；同一订单已有 pending 料理时只能等待，不能因场景里有多个同类厨具而重复制作。稀客和普客开锅前必须共用前端同一轮厨具预约表，按已摆放厨具快照计算同类厨具容量；容量不足时优先保留普客待处理订单，稀客只能等待料理开锅但仍可继续处理取酒或完成订单。
- 普客订单中的客人、料理和酒水名称只能使用本地数据仓库名称或明确可读文本。`GameData.CoreLanguage.LanguageBase`、`Il2Cpp*`、`GameData.*` 这类运行时类型名必须过滤掉；普客订单去重也不得依赖不稳定的客人文本。
- 自动化能力是实验性功能，必须由设置页总开关控制；总开关关闭时经营中页不显示自动化配置，也不执行任何自动化动作。稀客自动化按当前排序每轮最多推进 2 笔未满足订单，但完成订单写入每轮最多执行 1 笔，避免多订单同时修改运行时对象；普客订单处理必须额外由经营中自动化面板的“启用普客处理”子开关启用，开启后不保留手动处理按钮，由伴随窗口轮询自动执行。稀客阶段配置使用 `autoPrep*`，普客阶段配置使用 `autoNormal*`。普客只保留开始料理、收至保温箱和出错暂停，不要恢复取酒、直送订单、完成订单或 QTE 配置开关。子选项默认关闭但记忆用户上次配置。
...
- 自动化状态机只把实际动作视为进展，例如取酒、开锅、收取、写入订单或触发评价；不要把“选择订单”“匹配订单”这类前置成功当作进展，否则会掩盖真实卡住。目标料理/酒水长时间未进入容器时允许按订单状态回退到上一实际步骤重新执行，超过回退上限再暂停。C# pending 收取、开锅成功/失败和 pending 移除需要写入 `BepInEx/config/MystiaStewardCompanion/automation-jobs.log`，诊断日志必须有大小上限并且不能影响游戏流程。
