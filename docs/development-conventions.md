...
- `任务` 页读取当前进度可接但未接的交互任务，优先使用 `RunTimeScheduler.GetAvailableInteractMissionForCharacter()`；候选角色来源包括 `DataBaseDay.GetAllNPCKeys()`、`RunTimeDayScene.trackedNPCs` 和当前场景的 `CharacterConditionComponent`。场景任务交互物件可从 `MissionInteractConditionComponent` 只读显示为 `SceneInteractable`，不得调用 `TryTrigger...` 类会改变游戏状态的方法。读取失败必须显示诊断信息，不得回退到静态全任务列表误导用户。
