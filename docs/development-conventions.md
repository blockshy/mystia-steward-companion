...
- 自动开始料理默认跳过料理 QTE，保持原自动化速度但不累计音游数值或 Buff；打开原生 QTE 和自动模拟成功都属于实验性能力，必须只在 `自动开始料理` 开启时暴露。原生 QTE 打开成功后由游戏原生流程接管料理倒计时，不要提前调用倒计时；自动模拟成功应作为直接结算路径，不再同时打开原生面板，避免重复结算。运行时失败时返回诊断信息，不应中断已开始的料理。
