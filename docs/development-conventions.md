...
- 游戏角色、场景、挑战和规则名称优先采用当前游戏数据或程序集元数据中的正式名称。社区昵称不能成为业务标识。
...
- 推荐核心位于 `apps/companion/src/recommendation-engine/`。页面不复制候选生成、强制筛选或排序算法；密集计算通过 Worker 执行。
- C# Mod 不引用 TypeScript。两端通过版本化、本地认证的结构化 API 交换快照和命令。
- 游戏当前状态由 Mod 发布；前端不使用静态表、DOM 文本、HUD 可见性或超时来补全未知游戏状态。
...
- 平衡值、标签规则和功能说明写入结构化数据或领域模块，不散落在 JSX。
...
- 跨帧状态保存精确对象标识、轮次、修订号和必要数值，不缓存可失效的 IL2CPP wrapper。
- 游戏集合使用已验证的具体类型和读取器；集合形态、字段、值域、容量或对象标识不符合预期时整轮拒绝，不跳过错误项拼出不完整结果。
...
- 游戏调用异常和结果不确定的变更应原样进入失败状态；不得用日志成功、方法进入或预期结果推断游戏写操作已经提交。

具体订单、自动化和 UI 资源归属分别见[订单捕获与生命周期](runtime-order-lifecycle.md)、[自动化运行时](automation-runtime.md)和[游戏 UI 集成](game-ui-integration.md)。

## 信息不足时停止原则

- 未知不等于否，部分成功不等于完整成功，UI 可见不代表拥有游戏对象的控制权。
- 只有完整、唯一、属于同一轮次且可复核的证据才能开放业务或游戏写操作；证据缺失时保持等待、暂停或不可用。
- 不通过名称、文本、路径、列表位置、托管哈希、浮点容差、固定延迟或扫描整个场景猜测对象标识。
- 不用兼容性堆叠掩盖信息不足。无法确认的新来源应先记录诊断、补充反编译或实机证据，再替换旧路径。
- 不可逆操作前后都按该操作所属专题的精确标识重新检查；结果不确定时必须固定记录并阻止自动重复执行。

## 游戏分析证据

- 运行时行为依次用 metadata C#、BepInEx #783 interop、IDA/Hex-Rays 和实机日志交叉验证。
- 分析固定使用 `mods/bepinex/tools/il2cpp-analysis/` 的锁定工具链，输出放在仓库外；旧分析只做历史对比，不参与业务备用路径。
- Cpp2IL `dll_il_recovery` 中仅含 `ldnull; throw` 的方法不是可用源码，不得作为实现依据。
- 涉及游戏行为的改动必须在代码或专题文档中留下证据入口，并由对应运行时 smoke/audit 固定。
...
- 协议标识使用大小写敏感的规范值；序列化、签名和内容哈希必须稳定。
- 主设备配置版本、订单实例、自动化控制权和 UI 目标版本是不同概念，不能共用一个修订号或隐式转换。
...
2. 找到该领域唯一负责维护此规则的文档、实现入口和测试入口。
3. 删除被替代实现，再加入新实现；不要保留旧路径作为备用方案。
...
- 专题文档只记录本领域当前有效的规则。历史提交、已解决故障和阶段进度不进入长期文档。
