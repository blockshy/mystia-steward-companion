...
- 每项规则只由一份文档负责；其他文档只给出摘要和链接。
- 入口文档负责导航，不复制专题契约、完整命令矩阵或版本历史。
- 操作手册记录“怎样做”；架构和契约文档记录“边界是什么”；游戏机制文档记录“证据说明了什么”。
- 逐条断言以测试源码为准，[验证指南](validation-guide.md) 只说明改动范围与验证入口。
...
| [项目决策索引](repo-memory.md) | 当前稳定决策的短索引，以及各决策的主文档入口 |
...
| 文档 | 唯一职责 |
| --- | --- |
| [游戏数据提供器](runtime-provider.md) | 游戏静态目录、玩家状态、就绪条件、快照和失败状态 |
| [订单捕获与生命周期](runtime-order-lifecycle.md) | 夜间经营、普客/稀客捕获、强订单标识和最终状态记录 |
| [稀客调度与订单队列](rare-order-participation.md) | 默认关闭模块、配置/生效名单、显式队列位置、单订单/整组变更、优先插入规则与执行条件 |
| [自动化运行时](automation-runtime.md) | 自动化控制权、阶段执行条件、料理任务、暂停恢复和人工确认状态 |
...
| [Addressables 标签提取](addressables-tag-mapping-playbook.md) | 从游戏资源恢复标签 ID 映射的操作步骤 |
| [料理机制知识库](tmi-cooking-mechanics-knowledge-base.md) | 游戏评价、标签生成与压制机制 |
| [特殊经营游戏规则](special-business-scenes-notes.md) | 特殊场景的游戏原生规则与证据 |
| [特殊经营实现](special-business-implementation.md) | Mod 对特殊场景的推荐、对象标识和自动化策略 |
...
- 改运行时读取、订单、自动化、本地 API 或 UI 资源归属：只更新对应专题文档。
