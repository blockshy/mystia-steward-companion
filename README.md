...
- 经营中页自动检测当前稀客点单，按订单出现顺序显示桌号、料理 tag、酒水 tag 和推荐结果；严格推荐不足时会额外显示明确标注的喜好备选。
- 支持收藏稀客的常用料理方案和酒水；同一稀客再次出现相同点单 tag 时会优先显示可用收藏。
- 支持稀客订单专注模式，可调节料理/酒水显示数量并切换精简模式，减少滚动切换。
- 独立伴随窗口会记住上次的位置和大小，下次启动时自动恢复。
- 独立伴随窗口可在 `设置` 页调整背景透明度、置顶状态、主题、焦点切换行为、缺失厨具过滤、实验性游戏界面置顶和目标厨具高亮。
- 独立伴随窗口支持手柄导航，可用摇杆/十字键移动、确认、返回、切页、滚动和收藏推荐项。
- 设置页提供实验性自动化总开关；开启后可在经营中页按子选项自动处理当前排序第一笔稀客订单，并可选择料理 QTE 跳过、打开原生 QTE 或实验性模拟成功。
