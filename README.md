...
- **料理推荐**：按地区、普客、稀客和点单标签（Tag）推荐料理、加料与酒水。
- **收藏与自定义方案**：集中管理料理和酒水收藏，也可以为指定稀客配置固定推荐料理。
- **经营中助手**：实时显示普客订单、当前已启用的稀客订单、推荐方案、执行状态和必要诊断。
- **稀客调度**：在 `经营 -> 稀客调度` 统一管理开关、名单和订单队列。模块默认关闭；开启后可配置需要手动调度的稀客，其新订单默认暂停并从经营推荐隐藏，可按稀客或单笔订单暂停、队尾启用，或优先安排到当前高亮和所有正在自动处理的稀客订单之后，不改动游戏中的订单顺序。
...
自动化、游戏当前库存修改和部分游戏界面辅助会改变游戏状态。首次使用前建议备份存档，并先熟悉对应设置和帮助说明。

## Mod窗口页面一览

| 页面 | 用途 |
| --- | --- |
| `概览` | 客户端连接、主机网络、设备共享、运行状态。 |
| `推荐` | 普客、稀客、自定义、收藏、推荐规则。 |
| `经营` | 稀客和普客实时订单、稀客调度、游戏辅助，以及调试诊断；显示自动化摘要与跳转入口。 |
| `自动化` | 运行状态、订单重试与人工确认，以及执行配置。 |
| `工具` | 任务列表、稀客邀请、库存修改及低库存提醒。 |
| `设置` | 外观窗口、输入、更新、帮助，以及开启调试信息后显示的日志。 |
...
4. 在 `概览 -> 客户端` 确认已经连接，再到 `概览 -> 运行状态` 确认游戏数据已经就绪，然后进入推荐或经营页面。
5. 自动化、游戏界面辅助、任务列表、稀客邀请和稀客调度按需手动开启。
...
| `F10` | 注册成功后切换桌面鼠标穿透；可用状态见 `设置 -> 外观窗口`。 |
| `A` / `B` | 手柄确认 / 返回。 |
| `LB` / `RB` | 切换页面或页签。 |
| `LT` / `RT` | 滚动当前区域。 |
| `X` | 在推荐项上收藏或取消收藏。 |
...
| react-markdown | 版本说明的受限 Markdown 展示 | MIT | [remarkjs/react-markdown](https://github.com/remarkjs/react-markdown) |
...
Mod 会读取游戏当前状态；部分功能会修改游戏当前库存、调整窗口行为或执行实验性自动化操作。这些功能可能影响原始游戏体验，也可能在游戏更新、DLC 差异、特殊经营场景、其他 Mod 或系统环境变化时失效。
