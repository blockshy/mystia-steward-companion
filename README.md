...
- **稀客调度**：扩展功能中的独立模块，默认关闭；开启后可配置需要手动调度的稀客，其新订单默认暂停并从经营推荐隐藏，可按稀客或单笔订单暂停、队尾启用，或优先安排到当前高亮和所有正在自动处理的稀客订单之后，不改动游戏中的订单顺序。
...
| 页面 | 用途 |
| --- | --- |
| `概览` | 在连接子页管理客户端地址、Token 和重连，并查看游戏数据、库存摘要和常用操作。 |
| `推荐料理` | 查看普客、稀客推荐，维护自定义方案和收藏。 |
| `经营中` | 查看实时启用的订单、推荐方案和自动化状态；稀客调度开启后可管理稀客队列。 |
| `扩展功能` | 使用任务列表、稀客邀请、稀客调度和游戏当前库存修改。 |
| `设置` | 管理窗口、Mod 本机/LAN 监听与多设备配置、推荐、实验性功能、更新和帮助。 |
...
4. 在 `概览 -> 连接` 确认已经连接，再到 `概览 -> 状态` 确认游戏数据已经就绪，然后进入推荐或经营页面。
