...
- **经营中助手**：实时显示普客与当前参与的稀客订单、推荐方案、执行状态和必要诊断。
- **稀客调度**：扩展功能中的独立模块，默认关闭；开启后可配置需要手动调度的稀客，其新订单默认暂停并从经营推荐隐藏，可按稀客或单笔订单暂停、队尾启用，或优先安排到当前高亮和全部活动稀客料理任务之后，不改动游戏原生订单顺序。
...
| 页面 | 用途 |
| --- | --- |
| `概览` | 在连接子页管理客户端地址、Token 和重连，并查看运行时数据、库存摘要和常用操作。 |
| `推荐料理` | 查看普客、稀客推荐，维护自定义方案和收藏。 |
| `经营中` | 查看实时参与订单、推荐方案和自动化状态；稀客调度开启后可管理稀客参与队列。 |
| `扩展功能` | 使用任务列表、稀客邀请、稀客调度和运行时库存修改。 |
...
4. 在 `概览 -> 连接` 确认已经连接，再到 `概览 -> 状态` 确认运行时数据就绪，然后进入推荐或经营页面。
