...
12. 运行时稀客身份会归一化到本地 `customer_rare.json`。直接本地 ID 优先，其次尝试游戏映射、`StringId` / 名称别名和已知事件变体；当前已知变体包括 `Tewi_HardSell -> 因幡帝`、`Remilia -> 蕾米莉亚`。归一化后订单和活跃稀客都使用本地 ID，伴随窗口才能复用本地喜好与推荐规则。
