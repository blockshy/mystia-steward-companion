...
4. 对 `SpecialOrder` 读取 `RequestFoodTag`、`RequestBeverageTag`、`DeskCode` 和 `SpecialGuests`。其中两个 `Request*Tag` raw signed 数值属于订单身份；`0`、`-1` 等合法值必须原样保留，读取失败必须显式标记为缺失，不能用负数哨兵折叠两种状态。
...
6. 默认不依赖 BepInEx/Unity 日志识别点单。成功返回的 `GuestGroupController.PushToOrder` 或 `SetManualControllerOrderInternal` 是建立 order/controller/native-key 绑定的唯一入口；native key 只接受非零 IL2CPP 原生对象指针，读取不到时拒绝捕获，不得回退到 managed hash 或其他派生值。生成、HUD、伙伴与状态回调只能按同一 native key 更新或移除既有绑定。运行时捕获、Provider 和自动化都会将当前 `OrderBase` 通过共享具体类型解析入口唯一转换，再读取 `RequestFoodTag` / `RequestBeverageTag` 两项 raw signed 数值，与桌位和运行时原始稀客 ID 组成强身份；两项任一不可读即 fail-closed。归一化 `guestId` 只供推荐与特殊经营规则使用。展示唯一按 raw ID 查询完整运行时 signed Tag map；map miss 有界记录并拒绝投影，禁止 `GetOrderFoodText/GetOrderBevText`、override 委托链、`SpecialGuest.Get*TagText`、`ToString()`、`#id` 或文本回退。任一已绑定 observer（包括未送齐或非送达 context）必须在 context/fulfilled 门禁前比较同一 native slot + lifecycle 的两项 raw ID；任一冲突立即移除 capture、失效该 lifecycle，且不合并新值、不发布替代 identity/trace/order。该隔离不依赖 Provider 主动 fresh 扫描；只有后续成功原生创建绑定分配新 lifecycle 才可恢复。
7. 捕获业务就绪要求 `PushToOrder`、`SetManualControllerOrderInternal`、`RemoveFromOrder`、`EvaluateOrder`、`EvaulateManualOrder`、`CleanOrderInfo`、`RepellInternal` 七个精确 Hook 全部安装，并要求当前 Active generation 从开始时已被覆盖。Hook 在经营中途才补齐时，本场保持 fail-closed，下一场才可消费捕获。前两个成功创建 Hook 在 `HasEvaluated=false + PeekOrders()` 精确栈顶门禁后分配进程单调 `OrderLifecycleSequence`；后五个 Hook 只发布调用前锁存 lifecycle 的 scalar terminal receipt，评价来源为 `Evaluated`，其他来源为 `Removed`，旧 postfix 不得退休同 tuple 的新 lifecycle。Clear 清状态但不回退 sequence watermark。`EndDlc4SpecialManualOrder` 只移除 arrival event，不是订单退出边界。
...
9. 一般订单动作只接受已就绪精确捕获，并按桌位、`runtimeGuestId`、料理原始 Tag ID、酒水原始 Tag ID 强匹配；必要身份缺失或冲突时 fail-closed。三类订单自动化请求必须携带快照中的正 lifecycle sequence；自动送达、扣料、开锅、送达和评价前都要求 requested sequence、fresh capture sequence 与 active exact tuple 完全一致。自动送达前还要确认捕获对象仍是其 controller 的 `PeekOrders()` 当前栈顶且未满足；完成定位执行同一当前栈顶与强身份复核，并允许 `IsFullfilled=true` 进入评价。一般路径不得扫描 `GuestsManager` 回退；只有幽幽子三阶段和古明地恋 BOSS 两个显式命名路径在各自额外原生门禁下读取 manager 当前 controller，且仍必须取得同一 active lifecycle。
...
七 Hook 与当前经营 generation 就绪后，普客订单快照标记 `normalOrderMode=authoritativeCapture`：`NormalOrderRuntimeCapture` 的正 lifecycle 精确捕获是业务和执行权威，live `OrderController` / HUD 只追加没有捕获绑定的不可执行可见行；与 capture 具有同一 raw native key 的 unbound 行不得合并或覆盖 capture 状态。同 native tuple 的不同 lifecycle 必须独立，不能用 `Any` 合并 served/evaluated。不得按桌位、料理或酒水槽位重绑；单轮 HUD 空窗或读取错误不得过滤、隐藏或退休捕获。捕获未通过门禁时标记 `normalOrderMode=visibleFailClosed`，仅显示 HUD 行并禁止自动化，不扫描 `GuestsManager` 或队列建立启动绑定。terminal receipt 只保留标量身份，不持有订单/厨具 wrapper，也不代表 cooker controller lease。
