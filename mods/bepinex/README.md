...
- `经营中`：按订单出现顺序查看当前稀客、桌位、点单词条、满足点单的推荐料理、推荐加料和推荐酒水；严格满足点单的推荐不足时，会额外显示明确标注的“喜好备选（不满足点单）”。
...
- `设置`：调整窗口背景透明度（20% - 100%）、焦点切换行为、切换冷却时间、始终置顶、主题、手柄导航、缺失厨具过滤、实验性游戏界面置顶、目标厨具高亮，并控制实验性自动化总开关。
- `修改`：用 `+10` 和 `99` 快捷按钮修改当前运行时材料和酒水库存数量；修改后需要在游戏内保存才会持久化。
...
自动化默认关闭。需要使用时，先到 `设置` 页开启 `启用自动化（实验性）`，再到 `经营中` 页选择具体子功能：

- `自动完成订单`：当送餐盘中存在满足当前第一笔稀客订单的料理和酒水时，尝试完成该订单。
- `自动取酒`：按当前第一笔稀客订单的推荐酒水，从运行时库存放入送餐盘。
- `自动开始料理`：按当前第一笔稀客订单的推荐料理，选择可用厨具开始制作。
- `自动收取料理`：料理完成后尝试收入送餐盘。
- `只处理收藏配方`：只使用已收藏的料理方案和酒水。
- `出错时暂停`：遇到非临时错误后暂停当前订单，直到订单变化或重新开启。

开启 `自动开始料理` 后，可额外选择料理 QTE 行为：

- `跳过 QTE`：保持最快的自动料理流程，但不会累计音游数值或触发对应 Buff。
- `打开原生 QTE`：开始料理后尝试打开游戏原生料理音游面板，由玩家完成操作；若面板打开失败，会回退为跳过 QTE 并继续料理。
- `自动模拟成功（实验性）`：仅在 `打开原生 QTE` 下显示，会直接尝试触发游戏的 QTE 成功奖励入口，并跳过原生面板以避免重复结算；若游戏版本变动导致入口不可用，会在自动化状态中显示诊断信息。

自动化只处理伴随窗口当前排序第一笔稀客订单，不会全自动经营整晚流程。厨具占用、材料不足、送餐盘内容不匹配等临时状态会继续等待并重试；建议先在可回退的存档中测试。
