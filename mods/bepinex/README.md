...
自动化按伴随窗口当前排序处理稀客候选，每轮最多推进 2 笔未满足订单；为了避免一次轮询对多个运行时订单同时写入，`自动完成订单` 每轮最多完成 1 笔。普客会按首次出现顺序并发处理最多 3 笔未满足订单，每笔订单独立记录开始料理、收至保温箱和暂停状态。稀客和普客会共用本轮厨具预约表：Mod 会按当前已摆放厨具数量限制同类厨具的开锅请求，并在同类厨具容量不足时优先给待处理普客订单保留位置，稀客料理进入等待态但不会计作失败。已经进入制作中的普客料理会绑定到目标桌位，后续轮询不会因为同类厨具存在多个而重复开锅。普客自动化不处理酒水，也不会把料理直接送达订单；料理收至保温箱后，需要玩家按游戏原生流程送达。厨具占用、目标成品暂未进入容器、料理暂存容器不可用、运行时对象暂不可读等临时状态会继续等待并重试；目标料理或酒水长时间未出现时会回退到上一步重新执行，超过回退上限后才按设置暂停。材料不足、无可用推荐、收藏限定不满足等情况视为硬错误；建议先在可回退的存档中测试。
