...
- `自动收取料理`：料理完成后尝试进入普客保温/送达路径，不占用玩家送餐盘，并继续满足订单。
...
自动化只处理伴随窗口当前排序第一笔稀客订单；普客会按首次出现顺序并发处理最多 3 笔未满足订单，每笔订单独立记录取酒、开始料理和暂停状态。已经进入制作中的普客料理会绑定到目标桌位，后续轮询不会因为同类厨具存在多个而重复开锅。厨具占用、材料不足、送餐盘内容不匹配等临时状态会继续等待并重试；建议先在可回退的存档中测试。
