...
- `经营中`：查看当前稀客、稀客当前携带金钱、桌位、点单词条、满足点单的推荐料理、推荐加料和推荐酒水；订单默认按点单先后排列，也可在 `设置` 页切换为稀客分组，严格满足点单的推荐不足时，会额外显示明确标注的“喜好备选（不满足点单）”。页面底部会显示普通客订单诊断；开启实验性自动化总开关和“启用普通客处理”后，可按普通客独立阶段开关处理最早出现的未满足普通客订单。
...
自动化默认关闭。需要使用时，先到 `设置` 页开启 `启用自动化（实验性）`，再到 `经营中` 页分别选择稀客订单和普通客订单的具体子功能。两类订单的阶段开关互不影响。
...
普通客订单需要先开启 `启用普通客处理`，再单独配置：

- `直送酒水`：将当前第一笔普通客订单需要的酒水直接写入订单并扣减库存。
- `自动开始料理`：按普通客订单所需料理选择可用厨具开始制作。
- `自动收取料理`：料理完成后尝试进入普通客保温/送达路径，不占用玩家送餐盘，并继续满足订单。
- `出错时暂停`：遇到非临时错误后暂停普通客处理。
...
自动化只处理伴随窗口当前排序第一笔稀客订单；普通客处理最早出现的未满足订单，不会因后续新订单改变当前目标。厨具占用、材料不足、送餐盘内容不匹配等临时状态会继续等待并重试；建议先在可回退的存档中测试。
