...
自动化只处理伴随窗口当前排序第一笔稀客订单；普客会按首次出现顺序并发处理最多 3 笔未满足订单，每笔订单独立记录开始料理、收至保温箱和暂停状态。已经进入制作中的普客料理会绑定到目标桌位，后续轮询不会因为同类厨具存在多个而重复开锅。普客自动化不处理酒水，也不会把料理直接送达订单；料理收至保温箱后，需要玩家按游戏原生流程送达。厨具占用、目标成品暂未进入容器、料理暂存容器不可用、运行时对象暂不可读等临时状态会继续等待并重试；目标料理或酒水长时间未出现时会回退到上一步重新执行，超过回退上限后才按设置暂停。材料不足、无可用推荐、收藏限定不满足等情况视为硬错误；建议先在可回退的存档中测试。
