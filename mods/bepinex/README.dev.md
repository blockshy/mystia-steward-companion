...
- `GET /logs/settings`：读取总日志开关、总日志路径和单文件分片大小。
- `GET /logs/config?aggregateLog=true|false`：由伴随窗口回写总日志开关；`aggregateLog` 会即时注册或移除 BepInEx 全局日志监听器。
