...
- `GET /orders/normal/complete-first?...`：处理当前第一笔普通客订单。酒水会直接写入订单并扣减库存；料理优先匹配送餐盘成品，缺失时可按设置启动料理并自动收取，订单满足后才触发游戏评价流程。
...
普通客订单自动化仍是实验性功能。伴随窗口会显示当前 UI 订单里识别到的普通客桌位、料理、酒水和完成状态；设置页开启自动化总开关后，还需要在经营中自动化面板开启“启用普通客处理”，并至少开启直送酒水、自动开始料理或自动收取料理中的一个阶段，才会处理第一笔未满足普通客订单。当前实现不直接写保温箱：料理完成后收入送餐盘，再由下一次处理动作送达订单。
