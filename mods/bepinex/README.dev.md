...
- `GET /orders/normal/complete-first?...`：处理当前第一笔普客订单。酒水会直接写入订单并扣减库存；料理会按首次出现的普客订单绑定目标，完成后走普客保温/送达路径，不占用玩家送餐盘，订单满足后才触发游戏评价流程。
...
普客订单自动化仍是实验性功能。伴随窗口会显示当前 UI 订单里识别到的普客桌位、料理、酒水和完成状态；设置页开启自动化总开关后，还需要在经营中自动化面板开启“启用普客处理”，并至少开启直送酒水、自动开始料理或自动收取料理中的一个阶段，之后会自动处理按首次出现时间排序的未满足普客订单，不再需要点击手动处理按钮。普客料理完成后不进入玩家送餐盘，而是保留保温/送达中间态后写入对应订单，并在订单满足时触发评价，尽量贴近游戏原生进餐流程。
