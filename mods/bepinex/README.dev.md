...
- `runtime-static-data.log`：`DataBaseCharacter.GetAllMappedGuests()` 的事件变体稀客映射。
- `runtime-tags.log`：`DataBaseLanguage` 的料理/酒水标签文本、DLC 标签映射，以及 `DataBaseCore.TagRules`。
- `runtime-database-diff.log`：`DataBaseCore` 食材、酒水、菜品、料理运行时表，并附本地 JSON 名称、标签、价格对照。
- `runtime-guests.log`：`DataBaseCharacter` 普客、稀客、映射稀客、原始稀客映射和 `GuestFoodEasterEggData` 类型/简单字段。
- `runtime-izakayas.log`：`DataBaseCore.GetAllIzakayas()` 的经营场景标签、等级、普通/稀客池和刷新参数。
