...
- `GET /snapshot`：读取最新运行态快照。快照由 Unity 主线程按自动刷新节奏生成，网络线程只返回缓存 JSON。快照包含推荐状态、夜间稀客订单、当前可接任务和普通客订单诊断；任务读取有短缓存，候选来源包括全局 NPC、当前场景 NPC、`DaySceneMap`、跟踪交互物件、场景任务交互组件和未完成 `trackingMissions` fallback，并会尽量从 `RunTimeDayScene.trackedNPCs` 反查 NPC 所在场景；普通客诊断会扫描 HUD 订单和经营管理器桌位订单。
...
- `GET /orders/normal/complete-first?...`：处理当前第一笔普通客订单。酒水会直接写入订单并扣减库存；料理会按首次出现的普通客订单绑定目标，完成后走普通客保温/送达路径，不占用玩家送餐盘，订单满足后才触发游戏评价流程。
...
伴随窗口的自动化能力只在前端 `设置` 页总开关开启后运行，且稀客自动化只处理当前排序第一笔稀客订单。经营中订单排序支持点单顺序和稀客分组，必须同时影响经营中列表、专注模式、游戏界面置顶和自动化选单；料理/酒水排序配置会影响稀客页、经营中页、专注模式和自动化选单，新增排序项时需要同时覆盖这些入口。稀客与普通客自动化的阶段配置必须独立保存和独立传参：稀客使用 `autoPrep*` 配置，普通客使用 `autoNormal*` 配置，不得复用取酒、开始料理、收取、QTE 或出错暂停开关。子选项默认关闭并记忆用户上次配置。临时失败例如厨具占用、送餐盘缺少目标成品、运行时对象暂不可读，应保持可重试，不应永久停止自动任务；非临时错误在对应订单类型的 `出错时暂停` 开启时才暂停当前订单。

普通客订单自动化仍是实验性功能。伴随窗口会显示当前 UI 订单里识别到的普通客桌位、料理、酒水和完成状态；设置页开启自动化总开关后，还需要在经营中自动化面板开启“启用普通客处理”，并至少开启直送酒水、自动开始料理或自动收取料理中的一个阶段，才会处理最早出现的未满足普通客订单。普通客料理完成后不进入玩家送餐盘，而是写入对应订单的保温/送达路径并在订单满足时触发评价。
