...
- `GET /orders/normal/complete-first?...`：处理当前第一笔普客订单。酒水会通过订单送达状态写入并扣减库存；料理会按首次出现的普客订单绑定目标，完成后写入订单并通知 `FoodDelivered`，不占用玩家送餐盘；订单满足后才触发游戏评价流程。
...
伴随窗口的自动化能力只在前端 `设置` 页总开关开启后运行，且稀客自动化只处理当前排序第一笔稀客订单，普客自动化会在开启后按首次出现顺序并发处理最多 3 笔未满足普客订单。经营中订单排序支持点单顺序和稀客分组，必须同时影响经营中列表、专注模式、游戏界面置顶和自动化选单；料理/酒水排序配置会影响稀客页、经营中页、专注模式和自动化选单，新增排序项时需要同时覆盖这些入口。稀客与普客自动化的阶段配置必须独立保存和独立传参：稀客使用 `autoPrep*` 配置，普客使用 `autoNormal*` 配置，不得复用取酒、开始料理、收取、QTE 或出错暂停开关。普客自动化需要按订单 key 维护独立状态，非临时错误只暂停对应普客订单，不得暂停稀客自动化或其他普客订单；已进入制作中的普客料理必须绑定目标订单/桌位，后续轮询检测到 pending 后只能等待，不得在同类多个厨具上重复开始同一订单料理。子选项默认关闭并记忆用户上次配置。临时失败例如厨具占用、送餐盘缺少目标成品、运行时对象暂不可读，应保持可重试，不应永久停止自动任务；非临时错误在对应订单类型的 `出错时暂停` 开启时才暂停当前订单。

普客订单自动化仍是实验性功能。伴随窗口会显示当前 UI 订单里识别到的普客桌位、料理、酒水和完成状态；设置页开启自动化总开关后，还需要在经营中自动化面板开启“启用普客处理”，并至少开启直送酒水、自动开始料理或自动收取料理中的一个阶段，之后会自动处理按首次出现时间排序的未满足普客订单，不再需要点击手动处理按钮。普客酒水和料理送达时必须优先调用订单属性 setter，并通知 `PartnerManager.OnOrderBaseStatusUpdate` / `NotifySystemChanged` 的 `BeverageDelivered` 或 `FoodDelivered` 上下文，避免绕过游戏的进餐状态和 Buff 逻辑。
