...
- `GET /snapshot`：读取最新运行态快照。快照由 Unity 主线程按自动刷新节奏生成，网络线程只返回缓存 JSON。快照包含推荐状态、夜间稀客订单、当前可接任务和普通客订单诊断；任务读取有短缓存，候选来源包括全局 NPC、当前场景 NPC 和场景任务交互组件，普通客诊断会扫描 HUD 订单和经营管理器桌位订单。
...
伴随窗口的自动化能力只在前端 `设置` 页总开关开启后运行，且只处理当前排序第一笔稀客订单。经营中订单排序支持点单顺序和稀客分组，必须同时影响经营中列表、专注模式、游戏界面置顶和自动化选单；料理/酒水排序配置会影响稀客页、经营中页、专注模式和自动化选单，新增排序项时需要同时覆盖这些入口。子选项默认关闭并单独记忆：自动完成订单、自动取酒、自动开始料理、自动收取料理、只处理收藏配方和出错暂停。临时失败例如厨具占用、送餐盘缺少目标成品、运行时对象暂不可读，应保持可重试，不应永久停止自动任务；非临时错误在 `出错时暂停` 开启时才暂停当前订单。

普通客订单目前只做只读诊断。伴随窗口会显示当前 UI 订单里识别到的普通客桌位、料理、酒水和完成状态，用于后续验证“制作 -> 保温箱 -> 送达”和“酒水直接送达”的自动化入口；未确认这些运行时入口前，不要接入自动执行。
