...
- `POST /automation/cancel?target=commands|rare|normal|all`：由当前 automation control lease 所有者推进全局 command epoch，并在取消屏障后应用指定目标；`commands` 仅使排队/迟到命令失效并保留全部自动料理 job 与 automation control lease，`rare` / `normal` 只清理对应订单组并保留当前 automation control lease，`all` 清理全部 job 后释放控制权。缺少或非法 target 直接返回 `400`，不保留旧路由、缺参路径或 query 别名；响应返回规范 `target`、`commandEpoch`、`cancelledJobs`、`cancelledCommands` 和 `leaseReleased`。快照中的 `automationCancellationAppliedEpoch` 只在 Unity 主线程完成对应应用后推进，前端不得仅凭 HTTP ACK 恢复调度。
