...
internal static class SpecialBusinessContextRuleRegistry
{
    ...
    private static IReadOnlyDictionary<string, SpecialBusinessContextRule> BuildRules()
    {
        return new Dictionary<string, SpecialBusinessContextRule>(StringComparer.Ordinal)
        {
            [SpecialBusinessChallengeTypes.NotChallenge] = new(
                "standard",
                "当前不是特殊经营挑战，推荐和自动化按标准料理、酒水订单流程执行。",
                "使用标准推荐排序。",
                "使用标准自动化策略。",
                Array.Empty<int>()),
            ["Story_Basic"] = Trial(),
    ...
    private static SpecialBusinessContextRule StoryYuyukoChallenge()
    {
        return new SpecialBusinessContextRule(
            "boss",
            "剧情版 P2/P3 的自动化必须避开幽幽子的实际厌恶标签；P3 还必须复用游戏手动订单评价流程推进进度。",
            "P2 会优先保证安全高评价；P3 会先满足原订单料理和酒水，再优先选择无厌恶标签且可预测达到满意（Good）/完美（ExGood）的组合，并记录手动评价前后进度。",
            "剧情版幽幽子订单只有在确认当前客人控制器、送达目标、手动评价回调和三阶段评分回调后才会自动提交；回调流程不完整时会暂停评价，避免订单被消耗但进度不涨。",
            Array.Empty<int>());
    }
    ...
    private static SpecialBusinessContextRule RetakeYuyukoChallenge()
    {
        return new SpecialBusinessContextRule(
            ...
            "P2 会优先保证安全高评价；P3 会先满足原订单料理和酒水，再优先选择可达到满意（Good）/完美（ExGood）且无厌恶标签的料理、加料与酒水组合。",
            "重修版幽幽子订单只有在确认当前客人控制器、送达目标和对应 _50/_70 节点的游戏进度回调后才会调用评价流程；回调流程不完整时会暂停评价，避免订单被消耗但进度不涨。",
            Array.Empty<int>());
    }
    ...
    private static SpecialBusinessContextRule WackyCookingCompetition()
    {
        return new SpecialBusinessContextRule(
            "tag-target",
            "第一阶段按喜好标签命中数评价；第二阶段先要求完美（ExGood）再检查轮换目标标签；第三阶段分身另有改判规则，古明地恋本体使用揭示标签、护盾和投食进度机制。",
            "Mod 对第二阶段和第三阶段分身使用更严格的保守策略：保留原订单，并同时要求预估完美（ExGood）和当前目标标签；本体则按护盾与揭示状态选择。",
            "订单会标记特殊经营归属。开锅前检查阶段、目标标签和剩余时间；出锅后再校验成品，无法确认游戏评价流程时暂停。",
            Array.Empty<int>());
    }
    ...
    private static SpecialBusinessContextRule BloodPondHell()
    {
        return new SpecialBusinessContextRule(
            "tag-target",
            "游戏会随机指定两个料理标签；原订单成立后，同时命中两个目标标签才是完整挑战目标方案。",
            "仅对精确识别的血池地狱 BOSS 订单，在保持原订单成立的前提下选择同时满足两个动态料理标签的方案。",
            "仅在订单与客人均确认为 BOSS，且本场经营编号、原订单、双标签目标、实际成品和实时订单均严格复核后自动制作；酒水先送达。料理送达与订单完成均开启时按游戏原有流程精确结算，否则保留成品等待玩家处理。",
            Array.Empty<int>());
    }
    ...
    private static SpecialBusinessContextRule MizuchiStory()
    {
        return new SpecialBusinessContextRule(
            "mizuchi-story",
            "寻找瑞灵踪迹会随机让一名稀客表现为错误料理、错误酒水或错误对话；原始订单标签仍有效，只有当前被瑞灵附身的稀客需要在料理加料中加入噗噗呦果（材料 5002）。",
            "附身订单优先，并在满足原始料理、酒水订单的同时强制加入一份噗噗呦果；普通稀客继续按原始订单推荐，且禁止把噗噗呦果作为额外材料。",
            "仅在评价回调、稀客、随机控制类型、目标材料和订单记录均精确一致时接管；任何一项变化都会立即停止游戏写操作。",
    ...
    private static SpecialBusinessContextRule MizuchiTrial()
    {
        return new SpecialBusinessContextRule(
            "mizuchi-trial",
            "月都试炼会让一名稀客表现为错误料理、错误酒水或错误对话；原始订单标签仍有效，只有当前被瑞灵附身的稀客需要在料理加料中加入辣椒水（材料 5005）。",
            "附身订单优先，并在满足原始料理、酒水订单的同时强制加入一份辣椒水；普通稀客继续按原始订单推荐，且禁止把辣椒水作为额外材料。",
            "仅在评价回调、稀客、控制类型、目标材料和订单记录均精确一致时接管；任何一项变化都会立即停止游戏写操作。",
