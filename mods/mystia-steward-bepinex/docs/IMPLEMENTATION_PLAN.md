# 夜雀掌柜 BepInEx Mod 架构边界

## 目标

将夜雀掌柜推荐逻辑接入《东方夜雀食堂》运行时，提供可打包发布的 BepInEx IL2CPP Mod 和独立桌面伴随窗口。
...
1. Mod 从 `BepInEx/plugins/MystiaSteward/Data` 加载 JSON 静态数据。
2. 通过运行时反射读取游戏当前内存状态。
...
- `mods/mystia-steward-bepinex/tools/build-release.ps1`
